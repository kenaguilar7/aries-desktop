...
SELECT " + MapColumns + @"
FROM purchase_account_maps AS T0
WHERE T0.company_id = @CompanyId
LIMIT 1";

        public const string InsertMap = @"
INSERT INTO purchase_account_maps
  (company_id, inventory_account_id, tax_account_id, payable_account_id,
   cash_account_id, card_account_id, transfer_account_id, tax_rate, prices_include_tax,
   created_by, updated_by, active)
VALUES
  (@CompanyId, @InventoryAccountId, @TaxAccountId, @PayableAccountId,
   @CashAccountId, @CardAccountId, @TransferAccountId, @TaxRate, @PricesIncludeTax,
   @CreatedBy, @UpdatedBy, 1)";

        public const string UpdateMap = @"
UPDATE purchase_account_maps SET
...
T0.active AS Active";
        ...
        public const string SelectPostingByPurchase = @"
SELECT " + PostingColumns + @"
FROM purchase_postings AS T0
...
INSERT INTO purchase_postings
  (purchase_id, company_id, journal_entry_id, posted_at, totals_hash, created_by, updated_by, active)
VALUES
  (@PurchaseId, @CompanyId, @JournalEntryId, @PostedAt, @TotalsHash, @CreatedBy, @UpdatedBy, 1)";
    }
}
