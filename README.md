...
El API tiene CI propio ([`.github/workflows/ci-api.yml`](.github/workflows/ci-api.yml)): contrato HTTP + `docker build` (no publica). El deploy a ECS Express Mode (`us-east-2`) es a mano: [`.github/workflows/cd-api-test.yml`](.github/workflows/cd-api-test.yml) (servicio `aries-api-test`) y [`.github/workflows/cd-api-prod.yml`](.github/workflows/cd-api-prod.yml) (hay que escribir `production`). Secret extra en environments `test` / `Prod`: `ARIES_JWT_KEY`. El IAM de GitHub necesita ECR + ECS Express además de S3.

La UI Blazor (`src/web/Aries.Contabilidad`) tiene el mismo patrón: [`.github/workflows/ci-web.yml`](.github/workflows/ci-web.yml) (build + imagen nginx, no publica) y deploy a mano [`.github/workflows/cd-web-test.yml`](.github/workflows/cd-web-test.yml) (`aries-web-test`) / [`.github/workflows/cd-web-prod.yml`](.github/workflows/cd-web-prod.yml) (`aries-web-prod`, hay que escribir `production`). La imagen hornea `ApiSettings.BaseUrl` con el endpoint de `aries-api-test` / `aries-api-prod` (o `vars.ARIES_API_BASE_URL`). Publica primero el API (incluye CORS para `https://*.ecs.us-east-2.on.aws`); después la UI.
