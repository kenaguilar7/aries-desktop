...
namespace CapaPresentacion.cods
{
    ...
    public static class TreeViewAccountsBuilder
    {
            public static TreeNode[] BuildTreeView(this List<Account> LstCuentas)
            {
                ...
                foreach (Account item in LstCuentas)
                {
                    if (item.AccountType == AccountType.Cuenta_Titulo)
                    {
                        retorno.Add(CreateAccount(item, LstCuentas));
                    }
                }
            ...
            private static TreeNode CreateAccount(Account cuenta, List<Account> lst)
            {
                ...
                {
                    var l3 = (from c in lst where c.FatherAccount == cuenta.Id select c).ToArray<Account>();
                    if (l3.Length != 0)
                    {
                        TreeNode b = CreateAccount(item, lst);
                        retorno.Nodes.Add(b);
                    }
                }
